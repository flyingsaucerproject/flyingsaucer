...
public class MapperTest extends TestCase {
    ...
    public MapperTest(java.lang.String testName) {
        super(testName);
    }
    ...
    public void testChild() {
        ...
        net.homelinux.tobe.xhtmlrenderer.Ruleset r = new net.homelinux.tobe.xhtmlrenderer.Ruleset() {
            ...
            public java.util.Iterator getPropertyDeclarations() {
                ...
                l.add("childProperty");//for now
                return l.iterator();
            }
        };
        Selector s = new Selector(0, r, Selector.DESCENDANT_AXIS, "root");
        s.appendChainedSelector(Selector.CHILD_AXIS, "first");
        l.add(s);
        
        r = new net.homelinux.tobe.xhtmlrenderer.Ruleset() {
            public org.w3c.css.sac.SelectorList getSelectorList() {
                return null;//for now
            }
            ...
            public java.util.Iterator getPropertyDeclarations() {
                java.util.ArrayList l = new java.util.ArrayList();
                l.add("firstChildProperty");//for now
                return l.iterator();
            }
        };
        s = new Selector(0, r, Selector.DESCENDANT_AXIS, "first");
        s.appendChainedSelector(Selector.CHILD_AXIS, "first");
        l.add(s);
        ...
        r = new net.homelinux.tobe.xhtmlrenderer.Ruleset() {
            ...
            public java.util.Iterator getPropertyDeclarations() {
                java.util.ArrayList l = new java.util.ArrayList();
                l.add("noChildProperty");//for now
                return l.iterator();
            }
        };
        s = new Selector(0, r, Selector.DESCENDANT_AXIS, "root");
        s.appendChainedSelector(Selector.CHILD_AXIS, "second");
        l.add(s);
        
        Matcher myStyles = new Matcher();
        myStyles.setDocument(doc);
        myStyles.setAttributeResolver(null);
        myStyles.docMapper = myStyles.new Mapper(l);
        
        assertEquals("Should not lose any selectors", l.size(), myStyles.docMapper.axes.size());
        
        Element root = doc.getDocumentElement();
        java.util.Iterator i = myStyles.getMatchedPropertyDeclarations(root);
        assertEquals("Should be no properties for root", false, i.hasNext());
        
        NodeList n = root.getElementsByTagName("first");
        Element first = (Element) n.item(0);
        i = myStyles.getMatchedPropertyDeclarations(first);
        assertEquals("Should be one property for first", true, i.hasNext());
        String str = (String) i.next();
        assertEquals("first should have childProperty", "childProperty", str);
        assertEquals("Should be only one property for first", false, i.hasNext());
        
        n = first.getElementsByTagName("first");
        Element firstfirst = (Element) n.item(0);
        i = myStyles.getMatchedPropertyDeclarations(firstfirst);
        assertEquals("Should be one property for firstfirst", true, i.hasNext());
        str = (String) i.next();
        assertEquals("firstfirst should have firstChildProperty", "firstChildProperty", str);
        assertEquals("Should be only one property for firstfirst", false, i.hasNext());
        
        
        n = first.getElementsByTagName("second");
        Element second = (Element) n.item(0);
        i = myStyles.getMatchedPropertyDeclarations(second);
        assertEquals("Should be no properties for second", false, i.hasNext());
        Element secondsecond = (Element) n.item(1);
        i = myStyles.getMatchedPropertyDeclarations(secondsecond);
        assertEquals("Should be no properties for secondsecond", false, i.hasNext());
    }
