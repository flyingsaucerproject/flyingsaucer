...
class Selector {
     ...
     */
    Selector(int pos, net.homelinux.tobe.xhtmlrenderer.Ruleset parent, int axis, String elementName) {
        _parent = parent;
        _axis = axis;
        _name = elementName;
        _pos = pos;
    ...
    private Selector(int pos, int specificityB, int specificityC, int specificityD, net.homelinux.tobe.xhtmlrenderer.Ruleset parent, int axis, String elementName) {
    ...
    String getOrder() {
        ...
        String p = "00000"+_pos;
        return b.substring(b.length()-3)+c.substring(c.length()-3)+d.substring(d.length()-3)+p.substring(p.length()-5);
    }

    /** Check if the given Element matches this selector.
     * Note: the parser should give all class
     */
    public boolean matches(org.w3c.dom.Element e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver attRes) {
     ...
     * Note: the parser should give all class
     */
    public boolean matchesDynamic(org.w3c.dom.Element e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver attRes) {
            if(isPseudoClass(net.homelinux.tobe.xhtmlrenderer.AttributeResolver.LINK_PSEUDOCLASS))
            	if( attRes == null || !attRes.isPseudoClass(e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver.LINK_PSEUDOCLASS)) return false;
            if(isPseudoClass(net.homelinux.tobe.xhtmlrenderer.AttributeResolver.VISITED_PSEUDOCLASS))
            	if( attRes == null || !attRes.isPseudoClass(e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver.VISITED_PSEUDOCLASS)) return false;
            if(isPseudoClass(net.homelinux.tobe.xhtmlrenderer.AttributeResolver.ACTIVE_PSEUDOCLASS))
            	if( attRes == null || !attRes.isPseudoClass(e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver.ACTIVE_PSEUDOCLASS)) return false;
            if(isPseudoClass(net.homelinux.tobe.xhtmlrenderer.AttributeResolver.HOVER_PSEUDOCLASS))
            	if( attRes == null || !attRes.isPseudoClass(e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver.HOVER_PSEUDOCLASS)) return false;
            if(isPseudoClass(net.homelinux.tobe.xhtmlrenderer.AttributeResolver.FOCUS_PSEUDOCLASS))
            	if( attRes == null || !attRes.isPseudoClass(e, net.homelinux.tobe.xhtmlrenderer.AttributeResolver.FOCUS_PSEUDOCLASS)) return false;
            return true;
    }
   ...
   /** the CSS condition [attribute=value] */
    ...
    public net.homelinux.tobe.xhtmlrenderer.Ruleset getRuleset() {
        return _parent;
    }
    ...
    private net.homelinux.tobe.xhtmlrenderer.Ruleset _parent;
