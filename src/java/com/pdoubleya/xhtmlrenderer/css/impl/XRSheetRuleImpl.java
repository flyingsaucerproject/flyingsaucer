...
public abstract class XRSheetRuleImpl implements XRSheetRule {
  ...
  public static Iterator fromCSSStyleSheet( XRStyleSheet styleSheet ) {
    ...
    XRSheetRuleImpl xrr = null;
    List props[], reg, imp = null;
  ...
  /**
   * Returns the XRProperty given its propertyname. Note this is independent of
   * cascade, inherit, etc--it is the property as specified.
   *
   * @param propName  PARAM
   * @return          Returns
   */
  public XRProperty propertyByName( String propName ) {
   ...
   * non-important ones; however, in XR we separate these out to facilitate the
   * cascade logic, effectively making two rules with the same selector, one
   * important (higher priority) and one not.
   *
  ...
  private void pullPropertiesFromDOMRule(List propNames) {
    ...
    while ( names.hasNext()) {
      ...
      Iterator iter = XRPropertyImpl.fromCSSPropertyDecl( decl, propName, seq++ );
      while ( iter.hasNext() ) {
