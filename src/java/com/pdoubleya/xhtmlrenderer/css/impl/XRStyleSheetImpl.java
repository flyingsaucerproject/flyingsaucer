...
public class XRStyleSheetImpl implements XRStyleSheet {
  ...
  /**
   * Constructor for the XRStyleSheetImpl object. For a description of sequence,
   * see comments on XRStyleSheet interface.
   *
   * @param sheet   PARAM
   * @param origin  PARAM
   * @param seq     PARAM
  ...
  public Iterator rules() {
    return _rulesBySelector.values().iterator();
  }
   ...
   * @return          Returns
   */
  public XRStyleRule ruleBySelector( String selector, boolean important ) {
    return (XRStyleRule)_rulesBySelector.get( selectorImportantKey(selector, important));
  }

  /** Returns a string key including both the selector and its important property, usable for keying into a Map */
  ...
  */
  ...
  /**
   * Returns the CSSRuleList associated with the sheet--these are DOM rules, not
   * XRSheetRules!
   *
  ...
  private void pullRulesFromDOMStyleSheet() {
    CSSRuleList domRules = _domSheet.getCssRules();
