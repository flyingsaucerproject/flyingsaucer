 ...
 */
public class PaddingPropertyDeclarationFactory extends AbstractPropertyDeclarationFactory {
    /** Singleton instance. */
    private static PaddingPropertyDeclarationFactory _instance;
        ...
        /* if ( primVals.getCssValueType() == CSSValue.CSS_PRIMITIVE_VALUE ) {
            ...
            switch ( vList.getLength() ) {
                case 1:
                    ...
                    // not a list
                    throw new XRRuntimeException( "'padding' property has only one value, but SAC parser marked it as a value list." );
                case 2:
                    primitives = new CSSPrimitiveValue[]{
    ...
    public static synchronized PropertyDeclarationFactory instance() {
        if ( _instance == null ) {
            _instance = new PaddingPropertyDeclarationFactory();
        }
        return _instance;
    }
}// end class
