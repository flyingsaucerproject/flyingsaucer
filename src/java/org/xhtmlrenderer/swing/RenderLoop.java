...
public class RenderLoop implements Runnable {
	...
	public ReflowEvent collapseRepaintEvents(ReflowEvent evt) {
		...
		while(root.queue.isNextRepaintEvent()) {
			ReflowEvent evt2 = root.queue.peekNextRepaintEvent();
			//Uu.p("peeking at next repaint event: " + evt2);
			if(evt2.getType() == evt.MORE_BOXES_AVAILABLE) {
				changed = true;
