...
public class TableLayout extends BoxLayout {
    //TableBox table;
    private static final int fudge = 0;
    ...
    public Box layout(Context c, Element elem) {
        ...
        fixed_width -= table.margin.left + table.border.left + table.padding.left + 
            table.spacing.x +
            table.padding.right + table.border.right + table.margin.right;
        ...
        for(int i=0; i<nl.getLength(); i++) {
            if(nl.item(i).getNodeName().equals("td") || nl.item(i).getNodeName().equals("th")) {
                ...
                if(c.css.hasProperty(td,"width",false)) {
                    ...
                    col_widths[count] = (int)c.css.getFloatProperty(td,"width");
                    leftover_width -= col_widths[count];
                }
                count++;
            }
        }
        ...
        if(leftover_width > 0) {
            ...
            for(int i=0; i<col_count; i++) {
                ...
                if(col_widths[i] == -1) {
                    col_widths[i] = (leftover_width-table.spacing.x*col_count) / unset_count;
                }
            }
        }
        // debugging
        ...
        */
        ...
        prev_row.y = table.margin.top + table.border.top + table.padding.top - fudge;
            ...
            if(row.getNodeName().equals("tr")) {
                ...
                for(int j=0; j<cells.getLength(); j++) {
                    ...
                    if(cell.getNodeName().equals("td") || cell.getNodeName().equals("th")) {
                        ...
                        CellBox cellbox = new CellBox(0,0,col_widths[col_counter],0);
                        ...
                        cellbox.x = prev_cell.x + prev_cell.width + table.spacing.x + fudge;
                ...
                rowbox.x = + table.margin.left + table.border.left + table.padding.left;
                // y is prev row.y + prev row.height
                rowbox.y = prev_row.y + prev_row.height + table.spacing.y + fudge;
        ...
        table.height = prev_row.y + prev_row.height + table.spacing.y + 
            table.padding.bottom + table.border.bottom + table.margin.bottom;
    ...
    protected void paintTable(Context c, TableBox table) {
        ...
        for(int i=0; i<table.rows.size(); i++) {
            ...
            c.setExtents(new Rectangle(oe.x+row.x,oe.y+row.y,oe.width,oe.height));
