...
/**
 * Abstract test class for property explosion tests. It parses a stylesheet and lets you test the properties that are
 * then read from the sheet. Note, this does <b>not</b> test against a complete XHTML document. There is no selector
 * matching test, for example. All we are doing is testing the ability of XR to parse stylesheets into properties, above
 * the basic SAC parsing that we depend on otherwise. To do this, we test PropertyDeclarationFactories in their ability
 * to explode and clean properties read from the stylesheet. This class simplifies the testing  in that you just need to
 * define the PropertyDeclarationFactory instance you want to use, and the test cases. You must define {@link
 * #newPropertyDeclarationFactory()} and {@link #buildTestsMap()} in your subclass, as well as the standard {@link
 * #suite()} method for {@link TestCase}. The test map matches a given CSS selector with an associated property, which
 * may be a shorthand property, and then associates the expected properties that should be read when the input property
 * is parse. Samples are provided here, in method comments.
 *
 * @author Patrick Wright
 */
public abstract class AbstractPropertyExplosionTest extends TestCase {
    /**
     * The SAC CSSStyleSheet we have after parsing our input test data. Used to read all rules that were parsed from the
     * test input.
     */
    ...
    /**
     * The PropertyDeclarationFactory instance we use for exploding our properties
     */
    private PropertyDeclarationFactory factory;

    /**
     * Our Map of test data, used to build the parsable test input. The key is the selector, the value is a
     * two-dimensional Object array, first element the property assignment, second element a Map of exploded
    ...
    /**
     * The main test. Tests explosion of all test data in the testsMap map. This is purely generic: we walk through the
     * parsed selectors, and compare with the exploded values we expect to see, from the testsMap Map.
     */
    ...
    /**
     * This appends a set of tests to a test map you are building. You supply the tests Map, and the array of elements
     * you want permutations on. All possible permutations are added. Each permutation is then ready to be tested
     * against the test values Map which you supply. You can use this for some shorthand property tests where the
     * shorthand properties are not order-dependent. For example, the meaning of the shorthand border property is
     * dependent on the order of the values assigned to it. The meaning of font, and background, are not. To be able to
     * test parsing for those properties against a variation of inputs, this can save some time.
     * <p/>
     * Likely you would call this within {@link #buildTestsMap()} to add permutations to your tests.
     * <p/>
     ...
     * testVals);
     * </pre>
     * The <code>tests</code> map will then have all the permutations on inputs for the array we gave.
     *
    ...
    /**
     * Returns a CSS stylesheet built from the testsMap map. This is called during {@link #setUp()}. The tests Map is
     * built in {@link #buildTestsMap()}.
     *
     * @return A valid, parseable CSS string containing selectors and property declarations.
     */
    ...
    /**
     * Workaround. XRLog seems to cause problems when executed via Ant within jEdit, probably a treading problem or
     * conflict
     *
     * @param msg PARAM
     */
