...
public class DebugInlineBoxesAction extends AbstractAction {

    public DebugInlineBoxesAction() {
        super("Show Inline Boxes", SWT.CHECK, SWT.NONE, null);
    }

    public void run(Browser browser, MenuItem mi) {
        if (mi != null) {
            browser.getRenderer().getSharedContext()
                .setDebug_draw_inline_boxes(mi.getSelection());
